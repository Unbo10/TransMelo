...
class ObjArr:
    def __init__(self, capacity: int = 0):
        ...
        self.__arr = ctypes.cast(
            ctypes.CDLL(None).malloc(capacity * ctypes.sizeof(ctypes.py_object)),
...
if __name__ == "__main__":
    ...
    empty_arr: ObjArr = ObjArr()
    print("Empty:", empty_arr)
    arr_of_arrs: ObjArr = ObjArr(3)
    for i in range(3):
        temp: IntArr = IntArr(3)
        for j in range(3):
            temp[j] = j + i
        arr_of_arrs[i] = temp
    print("Size:", len(arr_of_arrs))
    print(arr_of_arrs)
